#!/bin/bash
# This script installs QEMU and KVM on Ubuntu. It is based on the official QEMU and KVM installation instructions for Ubuntu, which can be found here: https://help.ubuntu.com/community/KVM/Installation
# Ask user to confirm before proceeding, as this script will make changes to the system and may require a restart. The user can choose to skip the installation if they do not want to proceed.

# Call messages.sh to define font colours for outputting errors (red), warnings(yellow), information (cyan) or confirmation (green)
source ./lib/messages.sh
define_font_colours

EXIT_CODE=0
read -p $'This script will make changes to the system to install \033[1;32mQEMU/KVM\033[0m. Do you want to continue? (\033[1;32my\033[0m/\033[1;31mn\033[0m): ' -n 1 -r
echo ""
if [[ ! $REPLY =~ ^[Yy]$ && -n $REPLY ]]; then
  echo "QEMU/KVM installation skipped. You can run this script again later to install it."
  exit 0
fi

# Check if the CPU supports hardware virtualization (VT-x for Intel or AMD-V for AMD)
if [[ -z "$(lscpu | grep vmx)" && -z "$(lscpu | grep svm)" ]]; then
  echo -e "Your CPU does not support \033[1;31mhardware virtualization\033[0m (VT-x or AMD-V). QEMU/KVM may not work properly."
  read -n 1 -s -r -p $'Try enabling hardware virtualization in BIOS/UEFI settings, then try again.\nPress any key to continue\n'
  exit 1
fi

# Check if qemu and kvm are already installed
if command -v qemu-system-x86_64 &> /dev/null && command -v kvm &> /dev/null; then
  echo "QEMU and KVM are already installed. Skipping installation."
  exit 0
fi

# Install QEMU and KVM packages
sudo apt update
(( EXIT_CODE+=$? ))
sudo apt install qemu-system-x86 libvirt-daemon-system virtinst virt-manager virt-viewer ovmf swtpm qemu-utils libvirt-clients bridge-utils guestfs-tools libosinfo-bin tuned
(( EXIT_CODE+=$? ))

if [[ $EXIT_CODE -ne 0 ]]; then
  echo -e "There was an \033[1;31mERROR\033[0m installing \033[1;31mQEMU/KVM\033[0m. Please check the output above for details."
  read -n 1 -s -r -p $'Press any key to continue\n'
  exit 1
fi

sudo usermod -aG libvirt $USER
(( EXIT_CODE+=$? ))
sudo usermod -aG kvm $USER
(( EXIT_CODE+=$? ))

if [[ $EXIT_CODE -ne 0 ]]; then
  echo -e "There was an \033[1;31mERROR\033[0m adding your user to the libvirt and kvm groups. Please check the output above for details."
  read -n 1 -s -r -p $'Press any key to continue\n'
  exit 1
fi

echo "To check if KVM is working, run 'sudo kvm-ok'"
echo "To start the libvirt daemon, run 'sudo systemctl start libvirtd'"
echo "To check if the libvirt daemon is running, run 'sudo systemctl status libvirtd'"
echo "To enable the libvirt daemon to start on boot, run 'sudo systemctl enable libvirtd'"
echo "To manage virtual machines, you can use 'virt-manager' or 'virsh' commands."
echo "To start Tuned performance profile for host, run `sudo systemctl start tuned && sudo tuned-adm profile virtual-host && sudo tuned-adm active`"

read -n 1 -s -r -p $'Press any key to continue\n'
exit 0